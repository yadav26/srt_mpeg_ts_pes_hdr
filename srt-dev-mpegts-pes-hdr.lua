...
fields.ts_error_indicator = ProtoField.uint8("srt_dev.ts_error_indicator", "Error Indicator", base.HEX)
fields.ts_payload_start_indicator = ProtoField.uint8("srt_dev.ts_payload_start_indicator", "Payload Start Indicator", base.HEX)
fields.ts_priority = ProtoField.uint8("srt_dev.ts_priority", "Transport Priority", base.HEX)
fields.ts_packetid = ProtoField.uint32("srt_dev.ts_packetid", "Packet Id", base.HEX)
fields.ts_scrambling_control = ProtoField.uint8("srt_dev.ts_scrambling_control", "Scrambling Control", base.HEX)
fields.ts_adaptation_field_control = ProtoField.uint8("srt_dev.ts_adaptation_field_control", "Adaptation Field Control", base.HEX)
fields.ts_continuity_counter = ProtoField.uint8("srt_dev.ts_continuity_counter", "Continuity Control", base.HEX)
fields.ts_continuity_counter = ProtoField.uint8("srt_dev.ts_continuity_counter", "Continuity Control", base.HEX)
fields.ts_adap_len = ProtoField.uint8("srt_dev.ts_adap_len", "Adaptation Hdr Length", base.HEX)
fields.ts_adap_discontinuity_in = ProtoField.uint8("srt_dev.ts_adap_discontinuity_in", "Discontinuity Indicator", base.HEX)
fields.ts_adap_random_access_in = ProtoField.uint8("srt_dev.ts_adap_random_access_in", "Random Access Indicator", base.HEX)
fields.ts_adap_es_priority_in = ProtoField.uint8("srt_dev.ts_adap_es_priority_in", "ES Priority Indicator Indicator", base.HEX)
fields.ts_adap_pcr_flag = ProtoField.uint8("srt_dev.ts_adap_pcr_flag", "Pcr flag", base.HEX)
fields.ts_adap_opcr_flag = ProtoField.uint8("srt_dev.ts_adap_opcr_flag", "OPcr flag", base.HEX)
...
fields.FF_state = ProtoField.uint8("srt_dev.FF_state", "FF state", base.HEX, FF_state_select, 0x11)
...
function srt_dev.dissector (tvb, pinfo, tree)
	...
	if typebit == 1 then
		...
		pack_type_tree:add(fields.mpegts_packets, tvb(offset, 2))

		mpegts_packets = subtree:add(fields.mpegts_packets, tvb(offset, 1))
		
		packet_len =  tvb:len()
		for pos =32, packet_len, 188 do


			packet = mpegts_packets:add(fields.packet, tvb(offset, 1))
			mpegts_hdr = packet:add(fields.mpegts_hdr, tvb(offset, 1))
			
			
			ts_hdr_sync_byte = mpegts_hdr:add(fields.ts_hdr_sync_byte, tvb(offset, 1))
			offset = offset +1
			ts_error_indicator = mpegts_hdr:add(fields.ts_error_indicator, bit.rshift(bit.band(tvb(offset, 3):uint(), 0x800000),23))
			
			ts_payload_start_indicator = mpegts_hdr:add(fields.ts_payload_start_indicator, bit.rshift(bit.band(tvb(offset, 3):uint(), 0x400000),22))
			
			ts_priority = mpegts_hdr:add(fields.ts_priority, bit.rshift(bit.band(tvb(offset, 3):uint(), 0x200000),21))
			
			ts_packetid = mpegts_hdr:add(fields.ts_packetid, bit.rshift(bit.band(tvb(offset, 3):uint(), 0x1F0000),8))
			offset = offset +2
			ts_scrambling_control = mpegts_hdr:add(fields.ts_scrambling_control, bit.rshift(bit.band(tvb(offset, 1):uint(), 0xC0),6))

			adaptationPresent = bit.rshift(bit.band(tvb(offset, 1):uint(), 0x30),4)
			ts_adaptation_field_control = mpegts_hdr:add(fields.ts_adaptation_field_control, adaptationPresent)
			ts_continuity_counter = mpegts_hdr:add(fields.ts_continuity_counter, bit.band(tvb(offset, 1):uint(), 0x0F))
